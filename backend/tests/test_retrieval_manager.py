...
def test_manager_runs_requested_retrievers_and_maps_legacy_evidence():
    ...
    assert {"campus_rag", "download_search", "user_docs"}.issubset(bundle.retrievers)
    assert legacy["rag_hits"][0]["chunk_id"] == "campus-1"
    assert legacy["doc_hits"][0]["metadata"]["user_id"] == 7
    assert legacy["tool_results"][0]["items"][0]["file_url"] == "/late.pdf"
    assert legacy["retrieval_summary"]["independent_source_count"] == 3
...
def test_manager_rescues_when_first_pass_has_only_wrong_document_type():
    ...
    assert legacy["retrieval_summary"]["evidence_assessment"]["status"] == "sufficient"
